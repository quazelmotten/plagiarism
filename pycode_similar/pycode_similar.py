...
def main():
    ...
    # for i in func_ast_diff_list:
    #     print(f'{i.info_ref.func_node.lineno}:{i.info_ref.func_node.endlineno}, {i.info_candidate.func_node.lineno}:{i.info_candidate.func_node.endlineno}')
    #     print(i.plagiarism_percent)
    import difflib
    import os

    def generate_html_diff(func_ast_diff_list, pycode_list, output_file='diff_report.html'):
        """
        Generates an HTML file displaying the reference and candidate code side by side, with colors based on plagiarism.
        """
        # Helper function to determine the color based on plagiarism percentage
        def get_color(c):
            a = ["#774762FF", "#BA6E1DFF", "#D6BB3BFF", "#755028FF", "#F2DD78FF", "#205F4BFF", "#913914FF", "#585854FF", "#F0A430FF", "#768048FF", "#800000FF", "#1B3A54FF"]
            return a[c % len(a)]
        html_content = """
        <html>
        <head>
            <style>
                ...
                .code-block { padding: 10px; border: 1px solid #ddd; white-space: pre-wrap; word-wrap: break-word; width: 50%; }
        ...
        # Extract reference code lines from the pycode_list
        ref_code_lines = pycode_list[0][1].decode('utf-8').splitlines(True)

        # Initialize the coloring for the reference code
        ref_colored_lines = ['<div class="code-line">' + line + '</div>' for line in ref_code_lines]

        html_content += "".join(ref_colored_lines)  # Add reference code to the HTML
        ...
        # Extract candidate code lines from the pycode_list
        candidate_code_lines = pycode_list[1][1].decode('utf-8').splitlines(True)

        # Initialize the coloring for the candidate code
        candidate_colored_lines = ['<div class="code-line">' + line + '</div>' for line in candidate_code_lines]

        html_content += "".join(candidate_colored_lines)  # Add candidate code to the HTML

        # Now we go through the function diffs and color the lines accordingly
        current_color = 0
        for diff_info in func_ast_diff_list[::-1]:
            ref_start_line = diff_info.info_ref.func_node.lineno
            ref_end_line = diff_info.info_ref.func_node.endlineno
            candidate_start_line = diff_info.info_candidate.func_node.lineno
            candidate_end_line = diff_info.info_candidate.func_node.endlineno
            plagiarism_percent = diff_info.plagiarism_percent
            # Get the color for the current plagiarism percentage
            if plagiarism_percent < 0.5:
                color = 'white'
            else:
                color = get_color(current_color)
                current_color += 1

            # Apply color to the reference code lines
            for i in range(ref_start_line - 1, ref_end_line):  # 0-index adjustment
                ref_colored_lines[i] = f'<div class="code-line" style="background-color:{color}">{ref_code_lines[i]}</div>'
                # print(color, ref_code_lines[i])

            # Apply color to the candidate code lines
            for i in range(candidate_start_line - 1, candidate_end_line):  # 0-index adjustment
                candidate_colored_lines[i] = f'<div class="code-line" style="background-color:{color}">{candidate_code_lines[i]}</div>'
                # print(color, candidate_code_lines[i])
        ...
        html_content = """
            ...
            <body>
                ...
                <div class="container">
                    <div class="code-block">
                        <h3>Reference Code</h3>
                        """ + "".join(ref_colored_lines) + """
                    </div>
                    <div class="code-block">
                        <h3>Candidate Code</h3>
                        """ + "".join(candidate_colored_lines) + """
                    </div>
                </div>
            </body>
            </html>
        ...
        print(f"HTML diff report saved as {output_file}")

    generate_html_diff(func_ast_diff_list, pycode_list)
